...
private List<String> buildMetadataContent(
	...
	if (state.buildToolSelecting()) {
		...
		buildBuildToolSelectionContent(
				lines,
				state);
...
private void buildBuildToolSelectionContent(
		List<String> lines,
		CreateProjectState state) {
	
	List<io.github.avinashio.lazyspringboot.domain.project.BuildTool>
			buildTools =
			state.availableBuildTools();
	
	for (int index = 0;
		 index < buildTools.size();
		 index++) {
		
		String marker =
				index
						== state.selectedBuildToolIndex()
						? ">"
						: " ";
		...
		lines.add(
				" "
						...
						+ " "
						+ buildToolLabel(
						buildTools.get(index)));
	}
}

private String buildToolLabel(
		io.github.avinashio.lazyspringboot.domain.project.BuildTool
				buildTool) {
