...
public NavigationController(
		...
		ProjectSortController projectSortController, ProcessController processController, ProjectManagerController projectManagerController, EnvironmentController environmentController) {
...
public boolean handle(
	...
	switch (keyEvent.type()) {
		...
		case COMMAND_PALETTE -> commandPaletteController.open();
		...
		case UNDO -> dependencyUndoService.undo();
		
		case ACTIONS -> projectActionController.openActions(
...
private void handleCharacter(
	...
	switch (character) {
		...
		case 'a' -> projectActionController.openActions(
				uiState.selectedProject());
		
		case 'u' -> dependencyUndoService.undo();
		
		case 'n' -> createProjectController.open();
		
		case 'e' -> environmentController.open();
		
		case 'r' -> handleRefresh();
		
		case 'w' -> workspaceController.open();
		
		case 's' -> handleSort();
		
		case 'x' -> handleStopProject();
		
		case 'R' -> handleRestartProject();
		
		case 'l' -> handleProjectLogs();
		
		case 'p' -> projectManagerController.open();
...
private void handleUp() {
	...
	switch (uiState.panelFocus()) {
		...
		case DEPENDENCIES -> dependencyNavigation
									 .selectPreviousVisible();
...
private void handleEnter() {
	...
	switch (uiState.panelFocus()) {
		...
		case PROJECTS -> handleStartProject();
		
		case DEPENDENCIES -> uiState.startDependencyConfirmation();
